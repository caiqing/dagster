...
def test_reset_telemetry_profile(mocked_print):
    ...
    with mock.patch("dagster.core.telemetry.open", open_mock, create=True):
        ...
        assert mocked_print.mock_calls == [
            mock.call('Must set $DAGSTER_HOME environment variable to reset profile')
        ]
...
def test_enable_telemetry(mocked_print):
    ...
    with mock.patch("dagster.core.telemetry.open", open_mock, create=True):
        ...
        open_mock.assert_not_called()
        assert mocked_print.mock_calls == [
            mock.call('Must set $DAGSTER_HOME environment variable to enable telemetry')
        ]
...
def test_disable_telemetry(mocked_print):
    ...
    with mock.patch("dagster.core.telemetry.open", open_mock, create=True):
        ...
        open_mock.assert_not_called()
        assert mocked_print.mock_calls == [
            mock.call('Must set $DAGSTER_HOME environment variable to disable telemetry')
        ]
