...
def get_backfill_priority_from_args(kwargs):
    if kwargs.get('priority') is None:
        return None
    return int(kwargs.get('priority'))
...
@click.option(
    ...
    type=click.STRING,
    help=(
        'Specify a base run priority to for all runs kicked off by this backfill job'
        '\n\nExample: '
        'dagster pipeline backfill log_daily_stats --priority -1000'
    ),
)
...
def execute_backfill_command(cli_args, print_fn, instance=None):
    ...
    # Resolve priority
    priority = get_backfill_priority_from_args(cli_args)
    ...
    if noprompt or click.confirm(
        ...
        if priority is not None:
            run_tags['dagster/run_priority'] = priority
